...
SHADOW_COLUMNS = [
    ...
    "ind_cache_date", "tradable", "guard_reason",
]
...
def score_shadow(ticker, now=None):
    """Score one ticker, UNGATED. Returns a row dict or None on failure."""
    now = now or datetime.now(timezone.utc)
    ...
    combined = pl.combine_and_decide(step2, step3)
...
def resolve_shadow_outcomes(log_path=SHADOW_LOG, out_path=SHADOW_OUTCOMES,
    ...
    """Outcome resolution for shadow entries.

    Reuses live_tools.extract_episodes / resolve_episode unchanged, so a
    shadow episode is scored by exactly the rules a live one is. Costs zero
    Adanos: 4h klines only.
    """
    if not os.path.exists(log_path):
        print("no shadow log yet")
        return pd.DataFrame()
    return lt.resolve_outcomes(log_path=log_path, out_path=out_path,
...
def main():
    ...
    if a.mode == "run":
        ...
        resolve_shadow_outcomes(log_path=a.log)
